        ...
        <div class="col-lg-12 margins">
            <div class="ibox-content">
                ...
                <form method="POST" class="form-horizontal" id="form"
                      action="{{route('admin.music-genres.update', $genre->GenreID)}}" enctype="multipart/form-data"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-6">
                            {{-- <img style="max-width: 90%;" src="{{Config('constants.bucket.url').Config('filePaths.front.genre').$genre->Icon_Image}}" alt="">--}}
                    ...
                    <div class="form-group"><label class="col-sm-2 control-label">Genre Colour *</label>
                        <div class="col-sm-3">
                            <input type="color" class="form-control" name="color" value="{{in_array($genre->color, array("", null)) ? "#009B9E" : $genre->color }}">
                        </div>
                    </div>
                    <div class="form-group"><label class="col-sm-2 control-label">Tags</label>
                        <div class="col-sm-10">
                            <select class="select-simple-tag form-control" name="tags[]" multiple="multiple">
                                @if(isset($genre->tags) AND is_array($genre->tags))
                                    <?php foreach ($genre->tags as $key => $value):
                                        echo '<option value="' . $value . '" selected="selected">' . $value . '</option>';
                                    endforeach ?>
                                @endif
                            </select>
                            {{--<input type="text" class="form-control"  name="tags" value="{{$genre->tags}}">--}}
                        </div>
                    </div>
